...
class Robot(IdealRobot):
    ...
    def __init__(self, pose, agent=None, sensor=None, color="black", noise_per_meter=5, 
            noise_std=math.pi/60, bias_rate_stds=(0.1,0.1), expected_stuck_time=1e100, 
            expected_escape_time = 1e-100, expected_kidnap_time=1e100, 
            kidnap_range_x = (-5.0,5.0), kidnap_range_y = (-5.0,5.0)): #追加
...
class AoA(IdealAoA): ###noisesim_occlusion###
    def __init__(self, env_map,
                 distance_range=(0.5, 6.0),
                 direction_range=(-math.pi/3, math.pi/3),
                 distance_noise_rate=0.1, direction_noise=math.pi/90,
                 distance_bias_rate_stddev=0.1, direction_bias_stddev=math.pi/90,
                 phantom_prob=0.0, phantom_range_x=(0.0, 10.0), phantom_range_y=(0.0,10.0),
                 oversight_prob=0.1, occlusion_prob=0.0): #occlusion_prob追加
    ...
    def data(self, transmitter_pose):
        ...
        for ant in self.map.antennas:
            ...
            z = self.phantom(transmitter_pose, z)
            z = self.occlusion(z) #追加
            #z = self.oversight(z)
            if self.visible(z):
                z = self.bias(z)
...
if __name__ == '__main__':
    world = World(60, 0.1, debug=False)
    ...
    circling = Agent(0.2, 5.0/180*math.pi)

    #direction_noise=math.pi/90,
    #direction_bias_stddev=math.pi/90,
    aoa = AoA(m)

    r = Robot( np.array([4, 6, math.pi/5*6]).T, sensor=aoa, agent=circling, color="red")  # robot3は消しました
    world.append(r)
